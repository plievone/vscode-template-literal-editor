...
function activate(_context) {
    vscode.commands.registerTextEditorCommand('editor.openSubdocument', (editor) => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            yield findAndOpenTemplateUnderCursor(editor);
        }
        catch (err) {
            if (DEBUG) {
    ...
    function findAndOpenTemplateUnderCursor(editor) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (config.has(lang) && typeof config.get(lang) === 'string') {
                    ...
                    let matcher;
                    try {
                        matcher = new RegExp(config.get(lang), 'g');
                    }
                    catch (err) {
                        console.error('INVALID REGEX in templateLiteralEditor.regexes.%s: %s\n%s', lang, config.get(lang), err && err.stack || err);
                        yield vscode.window.showErrorMessage(`Invalid regex in templateLiteralEditor.regexes.${lang}: ${config.get(lang)}\n${err && err.stack || err}`);
                        throw err;
                    }
                    let match;
                ...
                if (templateStart !== 0) {
                    ...
                    console.warn('Template not found under cursor. If in error, please configure proper templateLiteralEditor.regexes.%s', doc.languageId);
                }
            }
            catch (err) {
                if (DEBUG) {
