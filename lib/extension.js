...
const DEBUG = false;
if (DEBUG) {
    process.on('unhandledRejection', (_p, reason) => {
...
function activate(_context) {
    vscode.commands.registerTextEditorCommand('editor.openSubdocument', (editor) => __awaiter(this, void 0, void 0, function* () {
        if (opening) {
            return;
        }
        opening = true;
        try {
            ...
            for (let handle of activeDocuments.values()) {
                if (handle.subdoc === editor.document) {
                    ...
                    yield handle.closeSubdocumentWithReason('Closed via toggling shortcut. This virtual document can be closed.');
                    // Early return
                    return;
                }
            }
            yield findAndOpenLiteralUnderCursor(editor);
        }
        catch (err) {
            if (DEBUG) {
    ...
    vscode.commands.registerTextEditorCommand('editor.closeSubdocuments', (_editor) => __awaiter(this, void 0, void 0, function* () {
        try {
            for (let handle of activeDocuments.values()) {
                // Alternatively could close only the document/subdocument that is open in the current editor, but let's close
                // them all for now.
                yield handle.closeSubdocumentWithReason('Closed via shortcut. This virtual document can be closed.');
            }
        }
        catch (err) {
            if (DEBUG) {
    ...
    function findAndOpenLiteralUnderCursor(editor) {
            ...
            try {
                ...
                if (config.has(doc.languageId) && typeof config.get(doc.languageId) === 'string') {
                    ...
                    try {
                        ...
                        yield vscode.window.showErrorMessage(`Invalid regex in templateLiteralEditor.regexes.${doc.languageId}: ${config.get(doc.languageId)}\n${err && err.stack || err}`);
                        throw err;
                    }
                    let match;
                ...
                if (templateStart !== 0) {
                    ...
                    const pickedLanguage = yield vscode.window.showQuickPick(sorted, { placeHolder: 'Open in Language Mode' });
                    if (pickedLanguage) {
                        previouslyPickedLanguage = pickedLanguage;
                        try {
                            yield activateSubdocument(pickedLanguage, editor, doc.positionAt(templateStart), doc.positionAt(templateEnd));
                        }
                        catch (err) {
                            if (DEBUG) {
                    ...
                    yield vscode.window.showWarningMessage(`Literal not found under cursor. If in error, please modify the source or templateLiteralEditor.regexes.${doc.languageId} configuration for your needs. Please also consider submitting your improved regexes to the vscode-template-literal-editor repository.`, 
    ...
    function activateSubdocument(language, editor, start, end) {
        return __awaiter(this, void 0, void 0, function* () {
            const doc = editor.document;
            ...
            if (activeDocuments.has(doc)) {
                yield activeDocuments.get(doc).closeSubdocumentWithReason('Reloading.');
            ...
            const subdoc = yield vscode.workspace.openTextDocument({ language, content: doc.getText(templateRange) });
            ...
            const subeditor = yield vscode.window.showTextDocument(subdoc, subeditorViewColumn);
            ...
            yield moveActiveCursorTo(targetPos);
             ...
             * Handlers
             */
            const documentCloseListener = vscode.workspace.onDidCloseTextDocument((closedDoc) => __awaiter(this, void 0, void 0, function* () {
                if (closedDoc === doc) {
                    try {
                        yield closeSubdocumentWithReason('Source document closed. This virtual document can be closed.');
                    }
                    catch (err) {
                        if (DEBUG) {
            ...
            const subdocumentCloseListener = vscode.workspace.onDidCloseTextDocument((closedDoc) => __awaiter(this, void 0, void 0, function* () {
                if (closedDoc === subdoc) {
                    try {
                        yield closeSubdocumentWithReason('Subdocument closed. This virtual document can be closed.');
                    }
                    catch (err) {
                        if (DEBUG) {
            ...
            function newSaveOverride() {
                return vscode.commands.registerTextEditorCommand('workbench.action.files.save', () => __awaiter(this, void 0, void 0, function* () {
                    try {
                        yield doc.save();
                    }
                    catch (err) {
            ...
            const throttledSyncToDocument = throttle(() => __awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    ...
                    const newEditor = yield vscode.window.showTextDocument(doc, editorViewColumn, /*preserveFocus*/ true);
                    const editOk = yield newEditor.edit(editBuilder => {
                        // We don't care about actual edits and partial templateRange synchronization,
                        // just copy everything in case there are changes
                    ...
                    try {
                        yield closeSubdocumentWithReason('Source document could not be synced with subdocument. This virtual editor can be closed.');
                    }
                    catch (err2) {
                        if (DEBUG) {
            ...
            const throttledSyncToSubdocument = throttle(() => __awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    // We have to always take a new reference to the editor, as it may have been hidden
                    // and a new editor may need to be created.
                    const newSubeditor = yield vscode.window.showTextDocument(subdoc, subeditorViewColumn, /*preserveFocus*/ true);
                    const editOk = yield newSubeditor.edit(editBuilder => {
                        // We don't care about actual edits and partial templateRange synchronization,
                    ...
                    try {
                        yield closeSubdocumentWithReason('Subdocument could not be synced with original document. This virtual editor can be closed.');
                    }
                    catch (err2) {
                        if (DEBUG) {
            ...
            function closeSubdocumentWithReason(reason) {
                return __awaiter(this, void 0, void 0, function* () {
                    try {
                        if (DEBUG) {
                ...
                return __awaiter(this, void 0, void 0, function* () {
                    if (vscode.workspace.textDocuments.indexOf(subdoc) >= 0) {
                        ...
                        try {
                            ...
                            // user may be typing.
                            yield vscode.window.showTextDocument(subdoc, subeditorViewColumn, /*preserveFocus*/ false);
                            ...
                            if (returnDoc && returnViewColumn) {
                                yield vscode.window.showTextDocument(returnDoc, returnViewColumn, /*preserveFocus*/ false);
                                // Artificial delay, to prevent "TextEditor disposed" warning (in Extension Development Host only).
                                yield shortDelay();
                                if (returnPos) {
                                    if (vscode.window.activeTextEditor && vscode.window.activeTextEditor.document === doc) {
                                        yield moveActiveCursorTo(returnPos);
...
function moveActiveCursorTo(targetPosition) {
    return __awaiter(this, void 0, void 0, function* () {
        const activeTextEditor = vscode.window.activeTextEditor;
...
function deactivate(_context) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            for (let handle of activeDocuments.values()) {
                yield handle.closeSubdocumentWithReason('Extension deactivated. This virtual document can be closed.');
            }
        }
        catch (err) {
            if (DEBUG) {
